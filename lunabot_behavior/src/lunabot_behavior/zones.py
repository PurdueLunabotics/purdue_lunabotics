...
def calc_point_from_apriltag(x: float, y: float, apriltag_pose_in_map: PoseStamped, is_sim: bool)-> 'tuple[float,float]':
    """
    Calculates the (x, y) point from the apriltag's location (given in the odom frame)
    """

    roll, pitch, yaw  = euler_from_quaternion([apriltag_pose_in_map.pose.orientation.x, apriltag_pose_in_map.pose.orientation.y, apriltag_pose_in_map.pose.orientation.z, apriltag_pose_in_map.pose.orientation.w])
    ...
    x0 = apriltag_pose_in_map.pose.position.x
    y0 = apriltag_pose_in_map.pose.position.y
...
def find_mining_zone(apriltag_pose_in_map: PoseStamped, is_sim: bool)->Zone:
    """ 
    ...
    top_left = calc_point_from_apriltag(DIST_X, DIST_Y + LENGTH_Y, apriltag_pose_in_map, is_sim)
    top_right = calc_point_from_apriltag(DIST_X + LENGTH_X, DIST_Y + LENGTH_Y, apriltag_pose_in_map, is_sim)
    bottom_left = calc_point_from_apriltag(DIST_X, DIST_Y, apriltag_pose_in_map, is_sim)
    bottom_right = calc_point_from_apriltag(DIST_X + LENGTH_X, DIST_Y, apriltag_pose_in_map, is_sim)
