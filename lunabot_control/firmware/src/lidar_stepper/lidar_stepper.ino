...
// Do do shifts 180-360 degrees, you could invert the OCR1B by doing an extra
// toggle using FOC

/// Note phase shifts will be rounded down to the next neared possible value so
/// the higher the frequency, the less phase shift resolution you get. At 8Mhz,
/// you can only have 0 or 180 degrees because there are only 2 clock ticks per
/// cycle.
...
int setWaveforms(unsigned long freq, int shift) {
    ...
    unsigned long clocks_per_toggle =
        (CLK / freq) / 2; // /2 becuase it takes 2 toggles to make a full wave

    ICR1 = clocks_per_toggle;

    unsigned long offset_clocks =
        (clocks_per_toggle * shift) / 180UL; // Do mult first to save precision
