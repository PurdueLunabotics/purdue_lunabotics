...
class PointToPoint:
    def __init__(self):
        ...
        self.MAX_ANGULAR_SPEED_DEG_PER_SEC = 180
        self.MAX_ANGULAR_SPEED_RAD_PER_SEC = np.deg2rad(
            self.MAX_ANGULAR_SPEED_RAD_PER_SEC)
        ...
        self.current_target_publisher = rospy.Publisher(
            '/ptp/current_target', Point, queue_size=10)
        self.velocity_publisher = rospy.Publisher(
            '/ptp/velocity', Twist, queue_size=10)
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if (self.robot_pose != (None, None, None) and self.last_pose != (None, None, None)):
            # linear velocity
    ...
    def translate_to_point(self, point):
        if self.target_point != (None, None):
            ...
            linear_error = np.linalg.norm(np.array(point) - current_location)
    ...
    def move_to_point(self, point):
        self.turn_to_point(point)
        self.translate_to_point(point)
    ...
    def run_node(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.move_to_point([2, 2, np.pi / 2])  # feed in dummy target (2,2)
