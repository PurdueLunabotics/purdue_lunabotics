...
class ManualController(Node):
    ...
    def __init__(self, **kwargs):
        ...
        self._max_speed = self.get_parameter("~max_speed").get_parameter_value().double_value # in rpm
        self.slow_drive_speed = 0.25
        self.fast_drive_speed = 1
        self.drive_speed_modifier = self.slow_drive_speed
        self.get_logger().info(f"Driving Speed: {self.drive_speed_modifier}")
    ...
    def joy_callback(self, joy):
        ...
        if joy.buttons[Buttons.X.value] == 1 and self.last_joy.buttons[Buttons.X.value] == 0:
            ...
            self.get_logger().info(f"Driving Direction: {self.driving_mode}")
        ...
        if joy.buttons[Buttons.START.value] == 1:
            ...
            if joy.buttons[Buttons.Y.value] == 1 and self.last_joy.buttons[Buttons.Y.value] == 0:  
                if (self.excavation_is_latched):
                    self.excavation_is_latched = False
                    self.get_logger().info("Excavation Released")
                else:
            ...
            effort_msg.dep_servo = int(joy.axes[Axes.DPAD_HORIZONTAL.value]+1 * self.SERVO_POS)/2
