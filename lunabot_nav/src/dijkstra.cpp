...
struct PriorityItem {
  ...
  PriorityItem(Vertex* vertex, Coord prev_coord, double cost, int goal_x, int goal_y) {
    this->vertex = vertex;
    this->cost = cost;
...
class DijkstraPlanner : public nav2_core::GlobalPlanner {
  ...
  public:
    ...
    nav_msgs::msg::Path createPlan(
      ...
      std::priority_queue<PriorityItem, std::vector<PriorityItem>, std::greater<PriorityItem>> queue;
      int goal_x, goal_y;
      costmap->worldToMapNoBounds(goal.pose.position.x, goal.pose.position.y, goal_x, goal_y);
      ...
      queue.push(PriorityItem(initial, Coord(-1, -1), 0, goal_x, goal_y));
      ...
      while (!queue.empty()) {
        ...
        if (item.vertex->coord.x == goal_x && item.vertex->coord.y == goal_y) {
          ...
          while (true) {
            ...
            costmap->mapToWorld((unsigned int) vertex.coord.x, (unsigned int) vertex.coord.y, pose.pose.position.x, pose.pose.position.y);
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if (isBlocked(Coord(new_x, new_y)) || vertex_list[new_x + new_y * width].visited) {
            continue;
          }

          double los_cost = getDistance(item.prev_coord.x - new_x, item.prev_coord.y - new_y);
          if (hasLineOfSight(item.prev_coord, Coord(new_x, new_y), los_cost)) {
            PriorityItem queue_item(
                ...
                vertex_list[item.prev_coord.x + item.prev_coord.y * width].cost + los_cost,
                goal_x, goal_y);
          ...
          PriorityItem queue_item(
              ...
              item.cost + 1 +
                  getTraversalCost(Coord(new_x, new_y)),
              goal_x, goal_y);
          queue.push(queue_item);
        }
    ...
    double getTraversalCost(Coord coord) {
      ...
      return 2.0 * curr_cost * curr_cost / (double) LETHAL_COST / (double) LETHAL_COST;
    }
