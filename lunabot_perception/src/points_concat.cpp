...
class PointsConcatFilter {
  ...
  private:
    ...
    typedef message_filters::sync_policies::ApproximateTime<PointCloudMsgT,
                                                            PointCloudMsgT>
        SyncPolicyT;
    ...
    for (size_t i = 0; i < PC_SIZE; ++i) {
        if (i < input_topics_.size()) {
            cloud_subscribers_[i] =
                new message_filters::Subscriber<PointCloudMsgT>(
                    nh_, input_topics_[i], 1);
        } else {
            cloud_subscribers_[i] =
                new message_filters::Subscriber<PointCloudMsgT>(
                    nh_, input_topics_[0], 1);
        }
    }
...
void PointsConcatFilter::pointcloud_callback(
    const PointCloudMsgT::ConstPtr &msg1,
    const PointCloudMsgT::ConstPtr &msg2) {
    ...
    try {
        for (size_t i = 0; i < input_topics_.size(); ++i) {
            ...
            tf_listener_.waitForTransform(output_frame_id_,
                                          msgs[i]->header.frame_id,
                                          ros::Time(0), ros::Duration(1.0));
            pcl_ros::transformPointCloud(
                output_frame_id_, ros::Time(0), *cloud_sources[i],
