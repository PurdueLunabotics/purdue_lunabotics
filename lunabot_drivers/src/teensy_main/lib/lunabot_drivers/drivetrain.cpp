...
namespace drivetrain { 
	...
	void run_drivetrain(const lunabot_msgs::Drivetrain& drive_msg, ros::NodeHandle& nh) {
		...
		*/
		...
		MotorDir left_dir = (vel_l > 0) ? CCW : CW; // wiring issue
		MotorDir right_dir = (vel_r > 0) ? CW : CCW; 
		...
		write_motor(drivetrain_cfg.left,abs(vel_l),left_dir);
		write_motor(drivetrain_cfg.right,abs(vel_r),right_dir);
	}
}