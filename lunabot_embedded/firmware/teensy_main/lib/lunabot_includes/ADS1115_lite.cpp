...
int16_t ADS1115_lite::getConversion() { // Wait for the conversion to complete
  ...
  Wire.write(ADS1115_REG_POINTER_CONVERT); // queue the data to be sent, in this case
  ...
  return ((Wire.read() << 8) | Wire.read()); // Read each byte.  Shift the first byte read 8 bits
...
bool ADS1115_lite::isConversionDone() {
  ...
  Wire.beginTransmission(_i2cAddress);    // Sets the Address of the ADS1115.
  Wire.write(ADS1115_REG_POINTER_CONFIG); // queue the data to be sent, in this case
                                          // modify the pointer register so that the
