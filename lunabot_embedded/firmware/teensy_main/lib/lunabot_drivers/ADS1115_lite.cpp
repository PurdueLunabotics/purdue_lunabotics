...
ADS1115_lite::ADS1115_lite(uint8_t i2cAddress) {
    ...
    _gain = ADS1115_REG_CONFIG_PGA_2_048V;  /* +/- 6.144V range (limited to VDD
                                               +0.3V max!) */
...
/*!
    @brief  Sets the gain and input voltage range.  Valid numbers:
         ...
         ADS1115_REG_CONFIG_PGA_2_048V   (0x0400)  // +/-2.048V range = Gain 2
   (default) ADS1115_REG_CONFIG_PGA_1_024V   (0x0600)  // +/-1.024V range = Gain
   4 ADS1115_REG_CONFIG_PGA_0_512V   (0x0800)  // +/-0.512V range = Gain 8
...
/*!
    @brief  Sets the mux.  Valid numbers:
        ADS1115_REG_CONFIG_MUX_DIFF_0_1 (0x0000)  // Differential P = AIN0, N =
   AIN1 (default) ADS1115_REG_CONFIG_MUX_DIFF_0_3 (0x1000)  // Differential P =
   AIN0, N = AIN3 ADS1115_REG_CONFIG_MUX_DIFF_1_3 (0x2000)  // Differential P =
   AIN1, N = AIN3 ADS1115_REG_CONFIG_MUX_DIFF_2_3 (0x3000)  // Differential P =
   AIN2, N = AIN3 ADS1115_REG_CONFIG_MUX_SINGLE_0 (0x4000)  // Single-ended AIN0
...
void ADS1115_lite::triggerConversion() {
    ...
    // Bits 0 through 4 deal with the comparator function.  The combined default
    // value for these bits is 0x0003
...
int16_t ADS1115_lite::getConversion() { // Wait for the conversion to complete
    ...
    Wire.write(
        ADS1115_REG_POINTER_CONVERT); // queue the data to be sent, in this case
                                      // modify the pointer register so that the
                                      // following RequestFrom reads the
                                      // conversion register
    Wire.endTransmission();           // Send the data

    Wire.requestFrom(_i2cAddress,
                     (uint8_t)2); // Request the 2 byte conversion register
    return ((Wire.read() << 8) |
            Wire.read()); // Read each byte.  Shift the first byte read 8 bits
                          // to the left and OR it with the second byte.
}
/**************************************************************************/
/*!
...
bool ADS1115_lite::isConversionDone() {
    ...
    Wire.beginTransmission(_i2cAddress); // Sets the Address of the ADS1115.
    Wire.write(
        ADS1115_REG_POINTER_CONFIG); // queue the data to be sent, in this case
                                     // modify the pointer register so that the
                                     // following RequestFrom reads the config
                                     // register
    Wire.endTransmission();          // Set the stop bit

    Wire.requestFrom(_i2cAddress, (uint8_t)2); // Request 2 byte config register
    return ((Wire.read() << 8) | Wire.read()) >>
           15; // Read 2 bytes.  Return the most signifagant bit
}
