...
#if defined(USBCON)
#define SabertoothTXPinSerial                                                  \
    Serial1 // Arduino Leonardo has TX->1 on Serial1, not Serial.
#else
...
/*!
\class Sabertooth
\brief Controls a %Sabertooth or %SyRen motor driver running in Packet Serial
mode.
*/
class Sabertooth {
  public:
    ...
    The driver address is set to the value given, and the Arduino TX serial port
    is used. \param address The driver address.
    */
    Sabertooth(byte address);

    /*!
    Initializes a new instance of the Sabertooth class.
    The driver address is set to the value given, and the specified serial port
    is used. \param address The driver address. \param port    The port to use.
    */
  ...
  public:
    ...
    */
    ...
    \param dontWait If false, a delay is added to give the driver time to start
    up.
    */
  ...
  public:
    ...
    \param value The voltage. The units of this value are driver-specific and
    are specified in the Packet Serial chapter of the driver's user manual.
    */
    void setMinVoltage(byte value) const;

    /*!
    Sets the maximum voltage.
    Maximum voltage is stored in EEPROM, so changes persist between power
    cycles. \param value The voltage. The units of this value are
    driver-specific and are specified in the Packet Serial chapter of the
    ...
    */
    ...
    \param baudRate The baud rate. This can be 2400, 9600, 19200, 38400, or on
    some drivers 115200.
    */
    ...
    \param value The deadband value.
                 Motor powers in the range [-deadband, deadband] will be
    considered in the deadband, and will not prevent the driver from entering
    nor cause the driver to leave an idle brake state. 0 resets to the default,
    which is 3.
    */
    ...
    */
    ...
    \param milliseconds The maximum time in milliseconds between packets. If
    this time is exceeded, the driver will stop the motors. This value is
    rounded up to the nearest 100 milliseconds. This library assumes the command
    value is in units of 100 milliseconds. This is true for most drivers, but
    not all. Check the packet serial chapter of the driver's user manual to make
    sure.
    */
