...
#define ADS1115_ADDRESS_ACT 0x4A //0x41 for new chip, 0x4A for old
#define ADS1115_ADDRESS_EXDEPDRIVE 0x4B //0x40 for new chip, 0x4B for old
/*=========================================================================*/
...
// NSingle Shot or Continuous Mode.  Cont mode is not used in this program but
// is preserved here for posterity
// #define ADS1115_REG_CONFIG_MODE_CONTIN  (0x0000)  // Continuous conversion
// mode
...
// Note that there is some overhead time of the I2C bus so the actual throughput
// is slightly less
#define ADS1115_REG_CONFIG_DR_8SPS                                             \
...
#define ADS1115_REG_CONFIG_DR_250SPS                                           \
    (0x00A0) // 250 SPS, or every 4ms, note that noise free resolution is
             // reduced to ~14.75-16bits, see table 2 in datasheet
#define ADS1115_REG_CONFIG_DR_475SPS                                           \
    (0x00C0) // 475 SPS, or every 2.1ms, note that noise free resolution is
             // reduced to ~14.3-15.5bits, see table 2 in datasheet
#define ADS1115_REG_CONFIG_DR_860SPS                                           \
    (0x00E0) // 860 SPS, or every 1.16ms, note that noise free resolution is
             // reduced to ~13.8-15bits, see table 2 in datasheet
...
class ADS1115_lite {
  ...
  public:
    ...
    bool testConnection(); // returns 1 if ADS1115 is properly connected, 0 if
                           // not connected.  There is no point in calling any
                           // other function is this doesn't return a 1

    void
    setGain(uint16_t gain); // Sets protected property and immediatly returns
    void
    setMux(uint16_t mux); // Sets Sets protected property and immediatly returns
    void setSampleRate(uint8_t rate); // Sets Sets protected property and
                                      // immediatly returns: sample rate

    void triggerConversion(
        void); // Triggers a single conversion with currently configured
               // settings in protected variables.  Immediately returns
    bool isConversionDone(); // Polls ADS1115 for the conversion done register.
                             // Returns 1 if conversion is finished, 0 if in the
                             // middle of conversion.
    int16_t getConversion(); // Polls ADS1115 for the conversion done register.
                             // Waits until it is finished and returns value.
};
